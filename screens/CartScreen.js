  ...
  return (
    <View className="bg-white flex-1">
      ...
      <View style={{backgroundColor:themeColors.bgColor(0.4)}} className="flex-row px-4 items-center">
        <Image source={require('../assets/images/ship1.png')} className="w-20 h-20 rounded-full"/>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{paddingBottom:50}} className="bg-white pt-5">
        {
            Object.entries(groupedItems).map(([key,items]) =>{
              let dish = items[0];
                return (
                    <View key={key} className="flex-row items-center space-x-3 py-2 px-4 rounded-3xl mx-2 mb-3 shadow-md" style={{backgroundColor:themeColors.bgColor(0.1)}}>
      ...
      <View style={{backgroundColor:themeColors.bgColor(0.5)}} className="p-6 px-8 rounded-t-3xl space-y-4">
        <View className="flex-row justify-between">
            <Text className="text-gray-700">Đơn hàng</Text>
        ...
        <View>
            <TouchableOpacity onPress={() => navigation.navigate('Order')} style={{backgroundColor:themeColors.bgColor(1)}} className="p-2 rounded-full">
