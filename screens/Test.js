...
export default function Test() {
  ...
  const search = (text) => {
    getFeaturedRestanrants().then(data=>{setFeaturedRestaurants(data)})
    let searchData = data.filter((item) => {
      return item.name.includes(text)
    });
    setFeaturedRestaurants(searchData)
  }
    ...
    <SafeAreaView style={SafeViewAndroid.AndroidSafeArea} className="bg-white">
      ...
      <View className="flex-row items-center space-x-2 px-4 pb-2">
        <View className="flex-row flex-1 items-center p-3 rounded-full border border-gray-300">
          ...
          <TextInput placeholder='Cửa hàng' className="ml-2 flex-1" onChangeText={(text) => search(text)}/>
